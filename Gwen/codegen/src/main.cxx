...
int main(int argc, char** argv) {
    ...
    try {
        ...
        //for(int i=0;i<count;i++) {
			...
			codegen_response_t* response = codegen_file("C:\\Users\\Gwendoline\\Documents\\Visual Studio 2010\\Projects\\Altaria\\codegen\\windows\\Release\\Count on Me.mp3", 10, 30, 0);
            char *output = make_json_string(response); // returns the json string
			printf("%s", output);
...
extern "C"
{
	__declspec(dllexport) char*	getFingerPrint(char* filename, int start_offset, int duration)
	{
		//codegen_response_t* response = codegen_file("C:\\Users\\Gwendoline\\Documents\\Visual Studio 2010\\Projects\\Altaria\\codegen\\windows\\Release\\Count on Me.mp3", 10, 30, 0);
		codegen_response_t* response = codegen_file(filename, start_offset, duration, 0);
		char *output = make_json_string(response); // returns the json string
		return output;
	}
}